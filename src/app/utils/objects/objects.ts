...
export class ObjectsUtil<T> {
   ...
   */
  private comparePropertyLength(fromObject: any, toObject: T): boolean {
   ...
   */
  private comparePropertyNames(fromObject: any, toObject: T) {
   ...
   * @param toObject
   */
  public iterateAndCopy(fromObject: any, toObject: T): T {
    ...
    if (namesComparisonFlag && lengthComparisonFlag) {
      ...
      for (const [keyAsProperty, valueOfProperty] of Object.entries(fromObject)) {
   ...
   */
  public dataObjectToArray(data: any): Array<T> {
    return Array.from(Object.keys(data), key => data[key]);
  }

  objectToInstance(theInstance: T, theFormObject: any): T {

    for (const [keyAsProperty, valueOfProperty] of Object.entries(theFormObject)) {

      if (theInstance.hasOwnProperty(keyAsProperty)) {
        theInstance[keyAsProperty] = valueOfProperty;
      }
